...
class BaseModel(Model):
    ...
    def select(cls, *fields):
        ...
        return super().select(*fields).where((cls.is_deleted == False) | (cls.is_deleted == None))
...
class Category(BaseModel):
    ...
    is_tab = BooleanField(default=False, verbose_name="是否显示首页")
...
class Goods(BaseModel):
    """
    商品  分布式的事务最好的解决方案 就是不要让分布式事务出现  ForeignKeyField所有前期引用外键
    """
    category = ForeignKeyField(Category, verbose_name="商品类目", on_delete='CASCADE')  # on_delete='CASCADE' 级联删除
...
class IndexAd(BaseModel):
    ...
    brands_one = ForeignKeyField(Brands, verbose_name="品牌1")
    brands_two = ForeignKeyField(Brands, verbose_name="品牌2")
    brands_three = ForeignKeyField(Brands, verbose_name="品牌3")


if __name__ == '__main__':
    settings.DB.create_tables([Category, Brands, Goods, GoodsCategoryBrand, Banner, IndexAd])
    index_ad = IndexAd.get(IndexAd.category == 135200)
    print(index_ad.id)
